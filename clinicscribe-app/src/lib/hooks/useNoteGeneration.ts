...
import type { SOAPNote, ConfidenceScores, MedicationDraft, FollowUpTask } from '@/lib/types';
...
export function useNoteGeneration() {
  ...
  const generate = useCallback(async (consultationId: string, transcript: string, patientContext?: string) => {
    setIsGenerating(true);
    setError(null);

    try {
      const data = await generateNote(consultationId, transcript, patientContext);
      setResult(data);
      return data;
    } catch (err) {
